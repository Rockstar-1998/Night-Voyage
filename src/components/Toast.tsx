...
export const NotificationContainer: Component = () => {
  return (
    <>
      {/* Toast 容器：右上角堆叠 */}
      <div class="fixed top-16 right-6 z-[4000] flex flex-col gap-2 pointer-events-none">
        <For each={toasts()}>
          {(toast) => {
            ...
            return (
              <div
                class={`pointer-events-auto flex items-start gap-3 max-w-sm rounded-xl border ${style.bg} ${style.border} ${style.text} px-4 py-3 shadow-2xl backdrop-blur-md animate-in fade-in slide-in-from-right-4 duration-300`}
                role="status"
              >
      ...
      <Show when={confirmState()}>
        {(state) => (
          <div class="fixed inset-0 z-[4000] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div
