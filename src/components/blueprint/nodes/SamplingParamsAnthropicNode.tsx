/**
 * Anthropic 版 SamplingParams 节点配置编辑器。
 *
 * 只暴露 Anthropic 支持的字段：temperature / max_tokens / top_p / stop[] /
 * thinking_enabled / thinking_budget_tokens。Anthropic 不支持
 * frequency_penalty / presence_penalty，故不在此出现。
 *
 * Constraints:
...
import type { AnthropicSamplingParamsConfig } from '../../../lib/blueprint/types';
...
export const SamplingParamsAnthropicNode: Component<
  NodeConfigComponentProps<AnthropicSamplingParamsConfig>
> = (props) => {
  const update = (updates: Partial<AnthropicSamplingParamsConfig>) =>
  ...
  return (
    <div class="space-y-4">
      <p class="text-[11px] text-mist-solid/45 leading-5">
        仅在当前会话协议为 <span class="text-orange-400/80">anthropic</span>{' '}
        时生效；chat_completions 会话下该节点不出参。
      </p>

      <div class="space-y-1">
        <label class={LABEL_CLASS}>temperature（0.0 – 1.0）</label>
        <input
          type="number"
          step="0.05"
          min="0"
