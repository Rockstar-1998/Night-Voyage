...
所有技术设计方案（`plans/`）、功能规范（`.trae/specs/`）和留痕记录（`Walkthrough/`）必须严格遵守以下文档编写准则：

1. **终态权威性（Canonical Master Spec，严禁废案元叙事）**：
   - 文档必须以第一方、自洽、权威的最终定案视角书写。
   - 严禁包含任何元对话、纠错历程或过时废案讨论（如“在进入实现前，首先纠正此前混淆……”、“此前讨论的废案是……”、“经用户指出修订……”）。
   - 确保接手者读到的就是第一天设计即确立的标准定案，不得看出多次修改妥协的痕迹。

2. **零废话与高信息密度（High-Density Substance，字可以多，但必须都是重点）**：
   - 杜绝空洞套话、过度修饰、概念包装与无意义的寒暄。
   - 可以详尽但每一句话必须具备实质工程价值（如具体数据结构、字段语义、算法复杂度、通信协议）。

3. **重机制与执行逻辑（Logic & Mechanisms First，杜绝代码堆砌）**：
   - 重点解释**怎么实现、如何运转**的执行逻辑，而不是仅给出孤立的代码块。
   - 必须清晰梳理生命周期时序、状态流转触发点、数据装配与拦截门禁、错误重试与降级防线。

4. **系统影响精准对齐（System Impact Alignment）**：
   - 方案设计必须直接回答并覆盖以下结构化维度：
     - **新增功能**：具体加哪些组件/能力，各自职责是什么；
     - **运作实现**：核心数据流、逻辑机制与具体交互流程；
     - **旧模块改动**：现有代码与接口是否变动，怎么变，如何兼容；
     - **系统协作**：与现有 Rust 后端服务、SQLite、前端宿主如何配合；
     - **蓝图与预设**：对工作流蓝图节点、预设管理的影响及适配方案。

5. **用户侧实操与调试体验闭环（User Experience, Blueprint Operations & Debugging）**：
   - 方案设计必须直接回答并覆盖终端用户与创作者的前台操作与调试闭环，杜绝只谈后台实现而忽略前台体验：
     - **用户如何使用**：界面交互入口、模式切换、状态感知与风控提示；
     - **蓝图实操连线**：新增节点规格、拓扑连线范例、表单参数配置与编译预览效果；
     - **调试与观测机制**：时序生命周期泳道、内存变量查看、门禁拦截（如禁词、D20）回放及人工干预路径。
